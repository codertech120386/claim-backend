...
async function getOrSetBrowserSession(mobile) {
  console.log('*********');
  console.log('inside getOrSetBrowserSession');
  console.log('*********');
  if (sessionMap.has(mobile)) {
    console.log('*********');
    console.log('if');
    console.log('*********');
    const sessionDetails = sessionMap.get(mobile);
    browser = sessionDetails.browser;
    page = sessionDetails.page;
    return { browser, page };
  } else {
    console.log('*********');
    console.log('else');
    console.log('*********');
    const browserDetails = await browserSessionManager.setupBrowser();
    browser = browserDetails.browser;
    page = browserDetails.page;

    sessionMap.set(mobile, { browser, page });
    return { browser, page };
  }
}

async function clickButton(page, name, selector) {
...
async function clickButtonWithMatchingText(page, text) {
  const buttons = await page.$$('button'); // Get all buttons

  for (let button of buttons) {
    const buttonText = await page.evaluate((el) => el.textContent, button); // Get text content of each button
    console.log('*********');
...
async function handleLogin(mobile) {
  const { browser, page } = await getOrSetBrowserSession(mobile);
  console.log('*********');
...
async function handleOTP(mobile, otp) {
  const { browser, page } = getOrSetBrowserSession(mobile);
  ...
  try {
    ...
    await clickButtonWithMatchingText(page, 'Close');
    ...
    await clickButtonWithMatchingText(page, 'Verify');
...
async function navigateToEPFOAndRaiseClaim(internalUserId) {
  try {
    ...
    const cardElement = await page.evaluateHandle(
      (el) => el.closest('.card-info.theme-switch-card'),
      epfoElement
    );
    await cardElement.click();

    // Wait for URL to change and contain 'dept'
    await page.waitForFunction('window.location.href.includes("dept")', {
      timeout: 30000,
    });
  ...
  try {
    ...
    try {
      ...
      if (popup) {
        ...
        if (
          popupText.includes('mobile number is not registered') ||
          ...
          return {
            success: false,
            message:
              'User not registered. Please complete registration process first.',
      ...
      try {
        const registrationForm = await page.waitForSelector(
          'mat-card.auth-card .headings strong',
          {
            visible: true,
            timeout: 5000,
          }
        ...
        if (registrationForm) {
          ...
          if (headingText.includes('Sign Up')) {
            ...
            return {
              success: false,
              message:
                'User not registered. Please register first before logging in.',
              requiresRegistration: true,
              error: 'Registration required',
            };
          }
...
async function verifyOTP(internalUserId, otp) {
  try {
    ...
    await page.waitForSelector(
      '.ng-otp-input-wrapper.wrapper.ng-star-inserted input',
      {
        visible: true,
        timeout: 30000,
      }
    ...
    await page.waitForSelector(
      '.btn.w-100.d-flex.justify-content-center.align-items-center:not([disabled])',
      {
        visible: true,
        timeout: 30000,
      }
    ...
    try {
      ...
      const errorPopup = await page.waitForSelector(
        '.mesg-popup .common-data',
        {
          visible: true,
          timeout: 5000,
        }
